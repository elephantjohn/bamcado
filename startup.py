...
def start_main_server():
    ...
    # 开始启动控制器
    logger.info(f"开始启动控制器：")
    controller_started = manager.Event()
    logger.info(f"启动控制器结束：")
    ...
    logger.info(f"填充process[controller]：")
    processes["controller"] = process
    logger.info(f"填充process[controller]结束")
    ...
    logger.info(f"填充process[openai_api]：")
    processes["openai_api"] = process
    logger.info(f"填充process[openai_api]结束")
    ...
    for model_name in args.model_name:
        ...
        if not config.get("online_api"):
            ...
            logger.info(f"填充process[model_worker]：processes[model_worker][{model_name}]")
            processes["model_worker"][model_name] = process
            logger.info(f"填充process[model_worker]：processes[model_worker][{model_name}]结束")
    ...
    for model_name in args.model_name:
        ...
        if (config.get("online_api")
            ...
            logger.info(f"填充process[online_api]：processes[online_api][{model_name}]")
    ...
    logger.info(f"填充process[api]")
    processes["api"] = process
    logger.info(f"填充process[api]结束")
