...
async def chat(query: str = Body(..., description="用户输入", examples=["你好"]),
    ...
    async def chat_iterator() -> AsyncIterable[str]:
        ...
        with open('/root/work/RAG/bamcad-chat-api/log.txt', 'a') as f:  # 'a' 表示追加模式
            f.write('after callbacks\n')
        ...
        if history:  # 优先使用前端传入的历史消息
            with open('/root/work/RAG/bamcad-chat-api/log.txt', 'a') as f:  # 'a' 表示追加模式
                f.write(' pass at and history_len.\n')
            pass
        elif conversation_id and history_len > 0:  # 前端要求从数据库取历史消息
            with open('/root/work/RAG/bamcad-chat-api/log.txt', 'a') as f:  # 'a' 表示追加模式
                f.write('history_len: ' + str(history_len) + '\n')  
                f.write('conversation_id: ' + str(conversation_id) + '\n')  
                f.write(' conversation_id and history_len > 0.\n')
            ...
            with open('/root/work/RAG/bamcad-chat-api/log.txt', 'a') as f:  # 'a' 表示追加模式
                f.write(' pass at and history_len > 0.\n')
            pass

        
        with open('/root/work/RAG/bamcad-chat-api/log.txt', 'a') as f:  # 'a' 表示追加模式
            f.write(' before model> 0.\n')
        #model = get_ChatOpenAI(
        #    model_name=model_name,
        #    temperature=temperature,
        #    max_tokens=max_tokens,
